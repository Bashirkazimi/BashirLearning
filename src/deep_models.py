...
def atrous_dw_conv(input_tensor, filter_size, kernel, stride, rate):
    """
    atrous depthwise separable convolution with given filter and kernel size and
    stride and rate
    :param input_tensor: input tensor
    :type input_tensor: keras tensor
    :param filter_size: number of output filters
...
def aspp_sep(x, xshape, depth=256):
    """
    Atrous Separable Spatial Pyramid Pooling for deeplab v3+
    :param x: inpute tensor
    :type x: keras tensor
    :param depth: number of channels
    :type depth: integer
    ...
    aspp2 = atrous_dw_conv(x, depth, 3, 1, 6)
    aspp3 = atrous_dw_conv(x, depth, 3, 1, 12)
    aspp4 = atrous_dw_conv(x, depth, 3, 1, 18)
...
def dl_entry_block_1(input_tensor):
    """
    Block 1 of entry flow in deeplab xception net
    :param input_tensor: input tensor
    :type input_tensor: keras tensor
    :return: output of block 1 entry flow
    :rtype: keras tensor
...
def dl_entry_sep_conv_block(input_tensor, filter, return_llf=False):
    """
    Sep conv Block of entry flow in deeplab xception net
    :param input_tensor: input tensor
    :type input_tensor: keras tensor
    :param filter: filters
    :type filter: integer
    :param return_llf: return low level features or not
    :type return_llf boolean
    :return: output of block sep_conv in entry flow
    :rtype: keras tensor(s)
    """
    x = mobile_dw_conv(input_tensor, filter, 3, 1)
    x = mobile_dw_conv(x, filter, 3, 1)
    skip = x
    x = mobile_dw_conv(x, filter, 3, 2)
...
def dl_entry_flow(input_tensor):
    """
    Entry flow of the deeplab xception network
    :param input_tensor: input tensor
    :type input_tensor: keras tensor
    :return: output of entry flow and low level features
    :rtype: two keras tensors
    """
    x = dl_entry_block_1(input_tensor)

    sep_conv_e_1 = dl_entry_sep_conv_block(x, 128)
    ...
    x = layers.Activation('relu')(x)
    sep_conv_e_2, skip = dl_entry_sep_conv_block(x, 256, True)
    side_conv2 = layers.Conv2D(
        256,
        1,
        2,
    ...
    x = layers.Activation('relu')(x)
    sep_conv_e_3 = dl_entry_sep_conv_block(x, 728)
    side_conv3 = layers.Conv2D(
        728,
        1,
        2,
    ...
    return side_conv3+sep_conv_e_3, skip


def dl_mid_flow(input_tensor):
    """
    middle flow block of xception for deeplab v3+
    :param input_tensor: input_tensor
    :type input_tensor: keras tensor
    :return: output of middle flow of xception
    :rtype: keras tensor
    ...
    for i in range(3):
        x = mobile_dw_conv(x, 728, 3, 1)

    return input_tensor + x


def dl_exit_flow(input_tensor, rates):
    """
    exit flow of xception for deeplab v3+
    :param input_tensor: input tensor
    :type input_tensor: keras tensor
    :param rates: dilation rates
    :type rates: list of integers
    :return: output of exit flow
    :rtype: keras tensor
    """
    x = layers.Activation('relu')(input_tensor)

    x = atrous_dw_conv(x, 728, 3, 1, rates[0])
    x = atrous_dw_conv(x, 1024, 3, 1, rates[0])
    x = atrous_dw_conv(x, 1024, 3, 1, rates[0])
    ...
    x = atrous_dw_conv(x, 1536, 3, 1, rates[1])
    x = atrous_dw_conv(x, 1536, 3, 1, rates[1])
    x = atrous_dw_conv(x, 2048, 3, 1, rates[1])

    return x


def deeplab_v3plus(input_shape=(299,299,3), num_classes=3):
    """
    Deeplab v3+ implementation based on https://arxiv.org/pdf/1802.02611.pdf
    :param input_shape: input shape
    :type input_shape: keras tensor
    :param num_classes: number of categories
    :type num_classes: integer
    :return: deeplab v3+ model
    :rtype: tf.keras.Model
    ...
    x, low_level_features = dl_entry_flow(input)

    for i in range(16):
        x = dl_mid_flow(x)

    x = dl_exit_flow(x, [1, 2])
    # x = aspp_sep(x, x.get_shape().as_list())
    ...
    # x = layers.Concatenate()([low_level_features, x])
    # x = mobile_dw_conv(x, 256, 3, 1)
    #
    # x = layers.Conv2D(
    #     num_classes,
